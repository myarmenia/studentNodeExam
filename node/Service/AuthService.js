...
const AuthService = {
    signUp: async (name,email,password,confirmPassword)=>{
        ...
        if(!findByName){
            if (!findByEmail) {
                ...
                return { message: "You Have Successfully Registered, Now You Can Loge In" };
                }else{
    ...
    signIn: async (email,password)=>{
        ...
        if(findUser){
            if(bcrypt.compareSync(password, findUser.password)){
                ...
                const tokenDelete = await Token.findOneAndDelete({user_id:findUser._id})
    ...
    refresh: async (refresh_token)=>{
        if(refresh_token){
            ...
            if(token){
                const newToken = jwt.verify(refresh_token, process.env.REFRESH_TOKEN, async (err,data)=>{
