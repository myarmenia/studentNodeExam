        ...
        if(allWines.length > 0){
            ...
            if(type && brand){
                ...
                if(brandArr.length> 1){
                    ...
                    brandArr.filter((b)=>{
                        if(el.about.manufacturer.name.toLowerCase() == b.toLowerCase()){
                        ...
                        if(filterByBrand.length > 0){
                            ...
                            typeArr.filter((t)=>el.type.toLowerCase() === t.toLowerCase()?filterByType.push(el):"")
                            })
                ...
                if(type){
                    ...
                    if(typeArr.length > 1){
                        const data = []
                        paginatedData.filter((el)=>{
                            typeArr.filter((t)=>el.type.toLowerCase() === t.toLowerCase()?data.push(el):"")
                        })
                        ...
                        const data = paginatedData.filter((el)=> el.type.toLowerCase() === typeArr[0].toLowerCase())
                        if(data.length > 0){
                            return data
                        }else{
                            return {message : "No Data Founded With This Type"}
                ...
                if(brand){
                    ...
                    if(brandArr.length> 1){
                        const data = []
                        paginatedData.filter((el)=>{
                            brandArr.filter((b)=>el.about.manufacturer.name.toLowerCase() === b.toLowerCase()?data.push(el):"")
                        })
                        ...
                        const data = paginatedData.filter((el)=> el.about.manufacturer.name.toLowerCase() === brandArr[0].toLowerCase())
                        if(data.length > 0){
                            return data
                        }else{
                            return {message: "No Data Founded With This Brand"}
                ...
                if(sort){
                    if(sort.toLowerCase() === "rating"){
                        const filteredData = paginatedData.filter((el)=>el.rating >= 4).sort((a,b)=>b.rating-a.rating)

                        return filteredData
                    }
                    if(sort.toLowerCase() === "sale"){
                        const filteredData = paginatedData.filter((el)=>el.sale > 120).sort((a,b)=>b.sale-a.sale)
