...
import { changePriceValueFrom, changePriceValueTo, setFilterIsOpen } from '../../Redux/Slices/Filter/FilterSlice'
import { productsGetThunk } from '../../Redux/Slices/Products/productsThunks'

const Filters = ({isBtn}) => {
    const {priceValueFrom,priceValueTo,filterIsOpen} = useSelector((state)=> state.filterData)
    ...
    useEffect(()=>{
        ...
        dispatch(filteredProductsThunk({type:activeTypesStr, brand: activeBrandsStr}))
    ...
    if(wines.length){
        ...
        wines.forEach((el) => {
            const findBrand = winesBrands.find((item) => item === el.about.manufacturer.name)
    ...
    <div className='filters_div'>
        <div className="filters_price">
            ...
            <div className="price_range">
                <span style={{left: parseInt(priceValueFrom)-2, width:`${rangeWidth}%`}}></span>
                <span>{priceValueFrom}$</span>
                <span>{priceValueTo}$</span>
            </div>
            <div className="price_inputs">
                <input type='number' min="0" max={`${priceValueTo-1}`} placeholder='from' value={priceValueFrom} onChange={handleFromValueChange} onKeyDown={(e) => { if (e.key === "Backspace" && priceValueFrom === 0) { e.preventDefault()}}}/>
                <input type='number' placeholder='to' value={priceValueTo} onChange={handleToValueChange} max="100" min="1" onKeyDown={(e) => {if (e.key === "Backspace" && priceValueTo === 0) {e.preventDefault()}}}/>
            </div>
        </div>
        <div className="wine_types_brands">
            <h3>Type</h3>
            <ul>
                {winesTypes.map((el,idx)=> <li key={idx}><input type="checkbox" onChange={()=>handleTypeCheckboxChange(el) }  checked={activeTypes.includes(el)}/>{el} </li>)}
            </ul>
        </div>
        <div className="wine_types_brands">
            <h3>Brand</h3>
            <ul>
                {winesBrands.map((el,idx)=> <li key={idx}> <input type="checkbox"  onChange={()=>handleBrandCheckboxChange(el) }  checked={activeBrands.includes(el)} /> {el} </li>)}
            </ul>
        </div>
        {
            isBtn? <button onClick={searchResults} className='filter_search_btn'>Search</button> : ""
        }
    </div>
  )
}
