...
const AccountPage = () => {
  const {userPageCurrent, activPat} = useSelector((state)=>state.filterData)
  ...
  barIsOpen === true ? document.body.style.overflow = "hidden" :document.body.style.overflow = "scroll"
        ...
        <div className="container">
            <div className="acoount_user_tab">
                  <div className="user_info">
                    ...
                    <div className="user_name" onClick={()=> dispatch(setBarIsOpen(!barIsOpen))}>
                        <h4>{user.name}</h4>
                        <h5>{userPageCurrent}</h5>
                        <i class="fa-solid fa-angles-right"  style={{transform: barIsOpen ? "rotateY(180deg)" :"rotateY(0deg)"}}></i>
                    </div>
                  </div>
            </div>
            <div className="account_bar_div" style={{left: barIsOpen? "2px" :"-1000px"}}>
